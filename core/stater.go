...
// DontCrack安卓(Android)Linux内核专用版
// https://github.com/FasterEdge/DontCrack4AndroidLinuxKernelSide
// tyza66
...
const rootMsg = "DontCrack_arm64-android By FasterEdge"
...
func Start(cfg config.Config) {
	...
	// 输出启动Logo和版本等欢迎信息
	log.Printf(logo)
	log.Printf("DontCrack_arm64-android for Android v%s 启动中...\n", cfg.Version)
...
// getShellCmd 构造一个基于本机可用 sh 的 exec.Cmd（用于 -pre 命令）。
// Android 默认 /system/bin/sh，部分系统在 /bin/sh；找不到则降级为 "sh"，由 PATH 决定。
...
func createCommand(cfg config.Config) *osexec.Cmd {
	...
	// Android 兼容：脚本统一通过本机可用的 sh 启动（不再硬编码 /bin/sh）
	shellPath := findShell()
	if shellPath == "" {
		shellPath = "sh"
	}
...
func startHealthProbe(cfg config.Config) {
	...
	go func() {
		...
		for {
			...
			shell := findShell()
			if shell == "" {
				shell = "sh" // 回退到 PATH 查找
			}
			ctx, cancel := context.WithTimeout(context.Background(), timeout)
			probe := osexec.CommandContext(ctx, shell, "-c", cfg.ProbeCmd)
			err := probe.Run()
			cancel()
			if err != nil {
				consecutiveFails++
